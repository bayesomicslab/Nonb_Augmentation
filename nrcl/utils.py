...
def load_data2(folder, win_size, n_bdna, n_nonb, nonb_ratio):
    ...
    for non_b in nonb_types:
        ...
        x_tr, x_val, y_tr, y_val = train_test_split(x_tr_val, y_tr_val, stratify=y_tr_val, train_size=0.5, random_state=42)
...
def load_data3(folder, nonb):
    # New Experiment_Data format: one csv per split (train/val/test) that embeds the
    # genomic metadata (chr, strand, window/motif coordinates) alongside the label and
    # the forward_*/reverse_* feature columns. There are no separate .bed files anymore,
    # so the bed dataframes are reconstructed from the metadata columns. The 'id' column
    # is aligned with each split's row index, which is what evaluation_exp() indexes by.
    meta_cols = ['chr', 'strand', 'win_start50', 'win_end50', 'motif_start', 'motif_end', 'features']

    def _load_split(split):
        df = pd.read_csv(os.path.join(folder, nonb + '_' + split + '.csv'))
        df = df.reset_index(drop=True)
        # feature frame: only the label + forward_*/reverse_* feature columns
        feature_cols = ['label'] + [c for c in df.columns
                                    if c.startswith('forward_') or c.startswith('reverse_')]
        data = df[feature_cols].copy()
        # bed frame: 'id' (= row index of data) + whatever metadata columns are present
        present_meta = [c for c in meta_cols if c in df.columns]
        bed = df[present_meta].copy()
        bed.insert(0, 'id', df.index)
        return data, bed

    global_train, train_bed = _load_split('train')
    global_val, val_bed = _load_split('val')
    global_test, test_bed = _load_split('test')
