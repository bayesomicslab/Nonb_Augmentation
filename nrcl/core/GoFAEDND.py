...
class GoFAEDND():
    ...
    def run(self):
        ...
        for epoch in range(self.epochs):
            ...
            for batch_idx, all_data in enumerate(self.train_loader):
                ...
                # Newer scipy returns an array from percentileofscore; stat is a
                # single scalar here, so collapse pval to a Python float.
                pval = float(np.asarray(pval).reshape(-1)[0])
        ...
        if self.unblock_test:
            ...
            if self.data_type != 'simulated':
                bed_file = os.path.join(self.exp_data_path, str(self.nonb_type) + '_test_bed.csv')
                if os.path.exists(bed_file):
                    test_bed = pd.read_csv(bed_file)
                    ...
                    print('No BED coordinate file found for {}; skipping filtered BED export.'.format(self.nonb_type), flush=True)
    ...
    def fit_test_novelty_stats(self, encoder_p1, encoder_p2, decoder):
        ...
        if self.use_test_rob_mean_cov:
            ...
            # MinCovDet (FastMCD) over the full test set is prohibitively slow on
            # the large ONT data (~24 min on 1.8M points) and statistically
            # unnecessary: a robust covariance in n_z dims is well estimated from
            # a large random subsample. Fit on a capped subsample, then apply the
            # Mahalanobis distances to ALL points below (unchanged).
            mcd_cap = getattr(self, 'mcd_max_fit', 150000)
            if latent_code.shape[0] > mcd_cap:
                rng = np.random.RandomState(self.seed)
                sub_idx = rng.choice(latent_code.shape[0], size=mcd_cap, replace=False)
                mcd_fit_latent = latent_code[sub_idx]
                print('MCD1: fitting robust covariance on a {}-point subsample of {} test B-DNA points.'.format(
                    mcd_cap, latent_code.shape[0]), flush=True)
            else:
                mcd_fit_latent = latent_code
            self.robust_cov_latent = MinCovDet(random_state=self.seed).fit(mcd_fit_latent)
            ...
            if test_bivdat.shape[0] > mcd_cap:
                rng2 = np.random.RandomState(self.seed)
                sub_idx2 = rng2.choice(test_bivdat.shape[0], size=mcd_cap, replace=False)
                mcd_fit_biv = test_bivdat[sub_idx2]
            else:
                mcd_fit_biv = test_bivdat
            self.MH2 = MinCovDet(random_state=self.seed).fit(mcd_fit_biv)
