...
def train_gp_nonb_sim(inp):
    ...
    if model_sel:
        ...
        best_param = gp_hyperparam_tuning(train, val, save_path)
        start = time.time()
        gp_model = GaussianProcessClassifier(kernel=best_param).fit(train_x, train_y)
        # gp_model = GaussianProcessClassifier(max_iter_predict=best_param).fit(train_x, train_y)
        duration = round(time.time() - start, 3)
    else:
        start = time.time()
        gp_model = GaussianProcessClassifier().fit(train_x, train_y)
        duration = round(time.time() - start, 3)
    
    print('Evaluation ... ')
    
    evaluate_classifiers(gp_model, test_x, test_y_true, dataset, method, nonb, nonb_ratio, save_path, winsize,
                         duration)
...
if __name__ == '__main__':
    ...
    gaussian_process_model(dataset, data_path, results_path, n_bdna, n_nonb, thread)
