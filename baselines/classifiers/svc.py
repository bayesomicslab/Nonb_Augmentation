...
def svc_model(dataset, folder, results_path, n_bdna, n_nonb, thread):
    ...
    if dataset == 'sim':
        ...
        nonb_ratios = [0.05, 0.1, 0.15,0.20,0.25] # [0.025, 0.05, 0.075, 0.1, 0.25, 0.5]
...
def train_svc_nonb_sim(inp):
    ...
    if model_sel:
        ...
        best_param = svc_hyperparam_tuning(train, val, save_path)
        start = time.time()
        svm_model = SVC(kernel=best_param, C=0.01, probability=True).fit(train_x, train_y)
        duration = round(time.time() - start, 3)
    else:
        start = time.time()
        svm_model = SVC(C=0.01, probability=True).fit(train_x, train_y)
        duration = round(time.time() - start, 3)
       
    print('Evaluation ... ')

    evaluate_classifiers(svm_model, test_x, test_y_true, dataset, method, nonb, nonb_ratio, save_path, winsize, duration)
...
if __name__ == '__main__':
    ...
    svc_model(dataset, data_path, results_path, n_bdna, n_nonb, thread)
