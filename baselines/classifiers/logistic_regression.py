...
def logistic_regression_model(dataset, folder, results_path, n_bdna, n_nonb, thread):
    ...
    if dataset == 'sim':
        ...
        pool = Pool(thread)
        pool.map(train_lr_nonb_sim, inputs)
        # for inp in inputs:
...
def train_lr_nonb_sim(inp):
    ...
    if model_sel:
        ...
        best_param = lr_hyperparam_tuning(train, val, save_path)
        start = time.time()
        lr_model = LogisticRegression(C=best_param).fit(train_x, train_y)
...
if __name__ == '__main__':
    ...
    logistic_regression_model(dataset, data_path, results_path, n_bdna, n_nonb, thread)
