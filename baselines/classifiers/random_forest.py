...
def random_forest_model(dataset, folder, results_path, n_bdna, n_nonb, thread):
    ...
    if dataset == 'sim':
        ...
        pool = Pool(thread)
        pool.map(train_rf_nonb_sim, inputs)
        # for inp in inputs:
...
def train_rf_nonb_sim(inp):
    ...
    if model_sel:
        ...
        best_param = rf_hyperparam_tuning(train, val, save_path)
        start = time.time()
        rf_model = RandomForestClassifier(criterion=best_param).fit(train_x, train_y)
    ...
    evaluate_classifiers(rf_model, test_x, test_y_true, dataset, method, nonb, nonb_ratio, save_path, winsize,
                         duration)
...
if __name__ == '__main__':
    ...
    random_forest_model(dataset, data_path, results_path, n_bdna, n_nonb, thread)
