...
def train_if_nonb_exp(inp):
    ...
    if model_sel:
        ...
        best_param = if_hyperparam_tuning(train, val, save_path)
        start = time.time()
        if_model = IsolationForest(max_samples=best_param).fit(train_x)
        duration = round(time.time() - start, 3)
    else:
        start = time.time()
        if_model = IsolationForest(max_samples=500).fit(train_x)
        duration = round(time.time() - start, 3)
...
def train_if_nonb_sim(inp):
    ...
    if model_sel:
        ...
        start = time.time()
        if_model = IsolationForest().fit(train_x)
        duration = round(time.time() - start, 3)
...
if __name__ == '__main__':
    ...
    isolation_forest_model(dataset, data_path, results_path, n_bdna, n_nonb, thread)
