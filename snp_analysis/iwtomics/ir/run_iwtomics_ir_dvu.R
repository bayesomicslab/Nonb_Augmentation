# IWTomics on Inverted Repeats -- detected vs undetected
# Run with Rscript, or block by block in an interactive session.
#
# Inputs (already produced by the notebook through cell 38):
#   $SNP_AF_BASE/results/ir_notebook_detected_run/
#     NCNR.InvertedRepeats.gnomAD.intersect              (detected motif x SNP)
#     NCNR.InvertedRepeats.gnomAD.undet.intersect        (undetected motif x SNP)
#     split_counts.txt                                   (N_MOTIF = N_DETECTED)
#
# Outputs:
#   InvertedRepeats_detected_vs_undetected_IWTomics_left_0.01.RData
#   InvertedRepeats_detected_vs_undetected_IWTomics_center_0.01.RData
#   InvertedRepeats_detected_vs_undetected_IWTomics_right_0.01.RData
#
# Side effect: appends N_UNDET (and N_UNDET_SPACER if available) to
# split_counts.txt so the plotting cell can read it back.
...
und_info  <- scan_loci_counts("NCNR.InvertedRepeats.gnomAD.undet.intersect")
...
det <- build_matrices("NCNR.InvertedRepeats.gnomAD.intersect",        N_DET)
und <- build_matrices("NCNR.InvertedRepeats.gnomAD.undet.intersect",  N_UNDET)
...
# the zero-spacer loci (~26% detected, ~17% undetected for IR) enter as
# all-zero rows and dilute the means: the spacer panel then shows a large
# detected-vs-undetected gap with almost no significant bins.
det_sp_mask <- spacer_loci_mask("NCNR.InvertedRepeats.split.bed",       nrow(det$Spacer))
und_sp_mask <- spacer_loci_mask("NCNR.InvertedRepeats.undet.split.bed", nrow(und$Spacer))
...
save(adj_pvalue_curve_left,
     file = "InvertedRepeats_detected_vs_undetected_IWTomics_left_0.01.RData")
...
save(adj_pvalue_curve_right,
     file = "InvertedRepeats_detected_vs_undetected_IWTomics_right_0.01.RData")
...
save(adj_pvalue_curve_center,
     file = "InvertedRepeats_detected_vs_undetected_IWTomics_center_0.01.RData")


# summary
files <- list.files(WORKDIR,
                    pattern = "InvertedRepeats_detected_vs_undetected_IWTomics.*\\.RData$",
                    full.names = TRUE)
