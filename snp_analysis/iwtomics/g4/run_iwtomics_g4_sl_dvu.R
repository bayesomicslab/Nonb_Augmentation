# IWTomics on G-Quadruplexes (stem/loop sub-annotation) -- detected vs undetected
...
#
# Inputs (produced by the notebook through the undetected-pool cell):
#   results/g4_stemloop_run/
#     G4sl.gnomAD.intersect                 (detected x SNP, bedtools -loj)
#     G4sl.gnomAD.undet.intersect           (undetected x SNP)
...
N_UNDET <- detect_n_loci("G4sl.gnomAD.undet.intersect")
...
und <- build_matrices("G4sl.gnomAD.undet.intersect",  N_UNDET)
...
run_iwt <- function(m_loci_x_bins, c_loci_x_bins, label) {
  ...
  cat(sprintf("%-10s  %3d bins, %5d det, %5d undet, %4d significant\n",
...
save(adj_pvalue_curve_left,
     file = "G4sl_detected_vs_undetected_IWTomics_left_0.01.RData")
...
save(adj_pvalue_curve_right,
     file = "G4sl_detected_vs_undetected_IWTomics_right_0.01.RData")
...
save(adj_pvalue_curve_center,
     file = "G4sl_detected_vs_undetected_IWTomics_center_0.01.RData")
